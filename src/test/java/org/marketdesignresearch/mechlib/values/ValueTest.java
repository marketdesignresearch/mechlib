...
public class ValueTest {
	...
	public void testXORValue() {
		...
		value.add(new BundleValue(BigDecimal.valueOf(13), new Bundle(Sets.newHashSet(new BundleEntry(D, 2)))));
	...
	private void checkValue(Bidder bidder, int amount, BundleEntry... bundleEntries) {
		assertThat(bidder.getValue(new Bundle(Sets.newHashSet(bundleEntries)))).isEqualTo(BigDecimal.valueOf(amount));
	}

	private void checkValue(Bidder bidder, int amount, Good... goods) {
		Set<BundleEntry> bundleEntries = new HashSet<>();
		for (Good good : goods)
			bundleEntries.add(new BundleEntry(good, 1));
		assertThat(bidder.getValue(new Bundle(Sets.newHashSet(bundleEntries)))).isEqualTo(BigDecimal.valueOf(amount));
	}
}
