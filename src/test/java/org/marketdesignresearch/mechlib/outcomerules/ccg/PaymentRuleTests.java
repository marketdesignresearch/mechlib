...
public class PaymentRuleTests {
	...
	public void testSimpleExample() {
		...
		SimpleXORDomain domain = new SimpleXORDomain(ImmutableList.of(westBidder1, westBidder2, globalBidder),
				ImmutableList.of(west));
		MechanismFactory equalNorm = new VariableAlgorithmCCGFactory(new XORBlockingCoalitionFinderFactory(),
	...
	public void testEqualRule() {
		...
		SimpleXORDomain domain = new SimpleXORDomain(ImmutableList.of(westBidder, eastBidder, globalBidder),
				ImmutableList.of(west, east));
		MechanismFactory equalNorm = new VariableNormCCGFactory(new BidsReferencePointFactory(),
	...
	public void testEqualRuleGeneric() {
		...
		OutcomeRule outcomeRule = equalNorm.getOutcomeRule(BundleExactValueBids.fromXORBidders(domain.getBidders()));
		Payment payment = outcomeRule.getPayment();
		assertThat(payment.paymentOf(westBidder2).getAmount()).as(
				"FIXME: This should produce the same result as above. Seems like something needs to be modified in the norms / reference points /...")
				.isEqualByComparingTo(BigDecimal.valueOf(1.75));
	...
	public void testEqualRulePerBidConstraintGenerator() {
		...
		SimpleXORDomain domain = new SimpleXORDomain(ImmutableList.of(westBidder, eastBidder, globalBidder),
				ImmutableList.of(west, east));
		MechanismFactory equalNorm = new VariableNormCCGFactory(new BidsReferencePointFactory(),
				List.of(new NormFactory(Norm.MANHATTAN, new EqualWeightsFactory(), Payment.ZERO),
						NormFactory.withEqualWeights(Norm.EUCLIDEAN)),
				ConstraintGenerationAlgorithm.PER_BID_CONSTRAINTS);
		OutcomeRule outcomeRule = equalNorm.getOutcomeRule(BundleExactValueBids.fromXORBidders(domain.getBidders()));
		Payment payment = outcomeRule.getPayment();
		assertThat(payment.paymentOf(eastBidder).getAmount()).isEqualByComparingTo(BigDecimal.valueOf(1.75));
		assertThat(payment.paymentOf(westBidder).getAmount()).isEqualByComparingTo(BigDecimal.valueOf(.25));
	...
	public void testLubinParkesNames() {
		ParameterizableCCGFactory mechFactory = new VariableNormCCGFactory(new BidsReferencePointFactory(),
				new NormFactory(Norm.MANHATTAN, new EqualWeightsFactory()),
				NormFactory.withEqualWeights(Norm.EUCLIDEAN));
	...
	public void paperExample() throws IOException {
		CPLEXUtils.SOLVER.exampleNormSolveParams();
		BundleValueBids<?> bids = BundleExactValueBids.fromXORBidders(
				SimpleXORDomain.fromCatsFile(Paths.get("src/test/resources/supersimple.txt")).getBidders());
		MechanismFactory quadratic = new VariableNormCCGFactory(new VCGReferencePointFactory(),
				NormFactory.withEqualWeights(Norm.MANHATTAN), NormFactory.withEqualWeights(Norm.EUCLIDEAN));
		MechanismFactory large = new VariableNormCCGFactory(new VCGReferencePointFactory(),
				NormFactory.withEqualWeights(Norm.MANHATTAN),
				new NormFactory(Norm.MANHATTAN, new PayoffWeightsFactory()));
		MechanismFactory small = new VariableNormCCGFactory(new VCGReferencePointFactory(),
				NormFactory.withEqualWeights(Norm.MANHATTAN),
				new NormFactory(Norm.MANHATTAN, new InversePayoffWeightsFactory()));
		MechanismFactory fractional = new VariableNormCCGFactory(new VCGReferencePointFactory(),
				NormFactory.withEqualWeights(Norm.MANHATTAN),
				new NormFactory(Norm.EUCLIDEAN, new InversePayoffWeightsFactory()));
