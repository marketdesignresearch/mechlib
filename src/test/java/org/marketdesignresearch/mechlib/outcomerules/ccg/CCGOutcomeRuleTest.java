...
import org.marketdesignresearch.mechlib.outcomerules.ccg.blockingallocation.IntermediateSolutionsAllocationFinderFactory;
import org.marketdesignresearch.mechlib.outcomerules.ccg.blockingallocation.MaxTraitorOrBlockingCoalitionFinderFactory;
import org.marketdesignresearch.mechlib.outcomerules.ccg.blockingallocation.MaxTraitorXORBlockingCoalitionFinderFactory;
import org.marketdesignresearch.mechlib.outcomerules.ccg.blockingallocation.MultiBlockingAllocationsDetermination;
...
public class CCGOutcomeRuleTest {
	...
	public static Collection<Object[]> outcomeRules() {
		MechanismFactory zeroMRCNorm = new VariableNormCCGFactory(new VCGReferencePointFactory(),
				new NormFactory(Norm.MANHATTAN, new EqualWeightsFactory(), Payment.ZERO),
				NormFactory.withEqualWeights(Norm.EUCLIDEAN));
		// MechanismFactory equalNorm = new VariableNormCCGFactory(new
		// BidsReferencePointFactory(), new NormFactory(Norm.MANHATTAN, new
		// EqualWeightsFactory(), Payment.ZERO),
		// NormFactory.withEqualWeights(Norm.EUCLIDEAN));

		MechanismFactory multipleSolutions = new VariableAlgorithmCCGFactory(
				new IntermediateSolutionsAllocationFinderFactory(MultiBlockingAllocationsDetermination.Mode.POOl_10),
				ConstraintGenerationAlgorithm.VALUE_SEPARABILITY);
		MechanismFactory orSeperability = new VariableAlgorithmCCGFactory(new OrStarBlockingCoalitionFinderFactory(),
				ConstraintGenerationAlgorithm.SEPARABILITY);
		MechanismFactory xorL2 = new VariableAlgorithmCCGFactory(new XORBlockingCoalitionFinderFactory(),
				ConstraintGenerationAlgorithm.STANDARD_CCG);
		MechanismFactory orL2 = new VariableAlgorithmCCGFactory(new OrStarBlockingCoalitionFinderFactory(),
				ConstraintGenerationAlgorithm.STANDARD_CCG);
		MechanismFactory orLinf = new VariableNormCCGFactory(new VCGReferencePointFactory(), Norm.MANHATTAN,
				Norm.MAXIMUM);
		MechanismFactory orLinfIterative = new VariableNormCCGFactory(new VCGReferencePointFactory(), Norm.MANHATTAN,
				Norm.ITERATIVE_MAXIMUM);
		MechanismFactory valueSeperation = new VariableAlgorithmCCGFactory(new OrStarBlockingCoalitionFinderFactory(),
				ConstraintGenerationAlgorithm.VALUE_SEPARABILITY);
		MechanismFactory xorLinf = new ConfigurableCCGFactory(
				new XORBlockingCoalitionFinderFactory(), new VCGReferencePointFactory(), ImmutableList
						.of(NormFactory.withEqualWeights(Norm.MANHATTAN), NormFactory.withEqualWeights(Norm.MAXIMUM)),
				ConstraintGenerationAlgorithm.SEPARABILITY);
		MechanismFactory xorSeperability = new VariableAlgorithmCCGFactory(new XORBlockingCoalitionFinderFactory(),
				ConstraintGenerationAlgorithm.SEPARABILITY);
		MechanismFactory orMaxTraitor = new VariableAlgorithmCCGFactory(
				new MaxTraitorOrBlockingCoalitionFinderFactory(), ConstraintGenerationAlgorithm.STANDARD_CCG);
		MechanismFactory xorMaxTraitor = new VariableAlgorithmCCGFactory(
				new MaxTraitorXORBlockingCoalitionFinderFactory(), ConstraintGenerationAlgorithm.STANDARD_CCG);
		MechanismFactory partialSeperability = new VariableAlgorithmCCGFactory(
				new OrStarBlockingCoalitionFinderFactory(), ConstraintGenerationAlgorithm.SEPARABILITY,
				ConstraintGenerationAlgorithm.PARTIAL_SEPARABILITY);
		MechanismFactory fullSeperability = new VariableAlgorithmCCGFactory(new OrStarBlockingCoalitionFinderFactory(),
				ConstraintGenerationAlgorithm.FULL);

		Object[][] data = new Object[][] { { zeroMRCNorm }, { orSeperability }, { multipleSolutions }, { xorL2 },
				{ orL2 }, { orLinf }, { orLinfIterative }, { valueSeperation }, { xorLinf }, { xorSeperability },
				{ orMaxTraitor }, { xorMaxTraitor }, { partialSeperability }, { fullSeperability } };
	...
	public void testExample2007Paper() throws IOException {
		...
		OutcomeRule wd = factory.getOutcomeRule(BundleExactValueBids.fromXORBidders(domain.getBidders()));
	...
	public void testExampleWurman2004Paper() throws IOException {
		...
		assertThat(result.paymentOf(bidder2).getAmount().add(result.paymentOf(bidder3).getAmount()).doubleValue())
				.isEqualTo(17.5, offset);
	...
	public void testQuadraticVsLinearExample() throws IOException {
		...
		assertThat(payment.paymentOf(domain.getBidder("SB" + 2)).getAmount().doubleValue()
				+ payment.paymentOf(domain.getBidder("SB" + 3)).getAmount().doubleValue()).isEqualTo(8, offset);
		System.out.println(payment.getMetaInfo());
