...
public class MipLoggingInstrumentation extends MipInstrumentation {
	...
	public void postMIP(String mipPurpose, IMIP mip, IMIPResult result, Allocation bestAllocation,
		...
		log.info("MIP Result -> Relative gap: {}%",
				BigDecimal.valueOf(result.getRelativeGap() * 100).setScale(4, RoundingMode.HALF_UP));
		...
		if (poolSolutions.size() > 0) {
			PoolSolution worst = poolSolutions.stream().min(Comparator.comparingDouble(PoolSolution::getObjectiveValue))
					.orElseThrow(NoSuchElementException::new);
			log.info("Worst Pool Solution -> Objective value: {}", worst.getObjectiveValue());
			log.info("Worst Pool Solution -> Relative pool gap: {}%",
					BigDecimal.valueOf(worst.getPoolRelativeGap() * 100).setScale(4, RoundingMode.HALF_UP));
			log.info("Worst Pool Solution -> Absolute pool gap: {}", worst.getPoolAbsoluteGap());
		}
		log.info("Best Allocation -> Total value: {}",
				bestAllocation.getTotalAllocationValue().setScale(4, RoundingMode.HALF_UP));
