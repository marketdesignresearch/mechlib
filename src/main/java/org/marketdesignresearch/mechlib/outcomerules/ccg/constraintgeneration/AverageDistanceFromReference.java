...
public class AverageDistanceFromReference implements Comparable<AverageDistanceFromReference> {
	...
	public BigDecimal getAverageDistance() {
		return blockedBidders.isEmpty() ? BigDecimal.ZERO
				: totalDistance.divide(BigDecimal.valueOf(blockedBidders.size()), MathContext.DECIMAL64);
	}

	public AverageDistanceFromReference subtract(AverageDistanceFromReference other) {
		return new AverageDistanceFromReference(totalDistance.subtract(other.totalDistance),
				ImmutableSet.copyOf(Sets.difference(blockedBidders, other.blockedBidders)));
	}
	...
	public String toString() {
		return "ADR[totalDistance=" + totalDistance + " ,blockedBidders=" + blockedBidders + " ,averageDistance="
				+ getAverageDistance() + "]";
	}
