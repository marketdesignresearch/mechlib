	...
	public Payment minimizeDistance(IMIP program) {
		...
		do {
			...
			for (Iterator<MaxDistanceConstraint> it = maxConstraints.iterator(); it.hasNext();) {
				...
				if (!(DoubleMath.fuzzyEquals(programResult.getDual(upConstraint), 0, 1e-6)
						&& DoubleMath.fuzzyEquals(programResult.getDual(downConstraint), 0, 1e-6))) {
					// fix payments maybe change to LEQ
					Constraint holdPayments = new Constraint(CompareType.EQ,
							programResult.getValue(maxDistanceConstraint.variable));
