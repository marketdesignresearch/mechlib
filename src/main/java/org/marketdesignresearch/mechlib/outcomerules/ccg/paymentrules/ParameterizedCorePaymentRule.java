...
public class ParameterizedCorePaymentRule extends BaseCorePaymentRule {
	...
	public ParameterizedCorePaymentRule(CorePaymentNorm primaryObjectiveNorm,
			CorePaymentNorm... secondaryObjectiveNorms) {
		...
		if (result == null) {
			try {
				...
				for (int i = 0; i < objectiveNorms.size() - 1; ++i) {
					PrimaryObjective primaryObjective = objectiveNorms.get(i)
							.getPrimaryObjectiveConstraint(programCopy);
