...
public class BlockedBidders {
	...
	private static final BlockedBidders EMPTY_BLOCKED_BIDDERS = new BlockedBidders(Collections.emptySet(),
			Collections.emptySet(), BigDecimal.ZERO);

	public BlockedBidders(Set<? extends Bidder> blockedBidders, Set<PotentialCoalition> blockingBidders,
			BigDecimal blockingCoalitionValue) {
	 ...
	 * 
	 * @return the coalitional value of the blocking bidders minus the total payment
	 *         of the traitors
	 */
	public BigDecimal getBlockedAmount(Payment lastPayment) {
		// Traitors:= bidders (W) \cap blockingBidders (C)
		BigDecimal traitorsPayment = blockingCoalition.stream().map(PotentialCoalition::getBidder)
				.map(lastPayment::paymentOf).map(BidderPayment::getAmount).reduce(BigDecimal.ZERO, BigDecimal::add);
