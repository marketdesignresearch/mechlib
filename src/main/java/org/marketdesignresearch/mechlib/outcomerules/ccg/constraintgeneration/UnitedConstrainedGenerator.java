...
public class UnitedConstrainedGenerator implements ConstraintGenerator {
	...
	public UnitedConstrainedGenerator(BundleValueBids<?> bids, Outcome referencePoint,
			Set<PartialConstraintGenerator> generatorAlgorithms, CorePaymentRule corePaymentRule) {
		...
		for (PotentialCoalition coalition : referencePoint.getAllocation().getPotentialCoalitions()) {
			...
			for (Good good : coalition.getBundle().getBundleEntries().stream().map(BundleEntry::getGood)
		...
		for (PartialConstraintGenerator particalConstraintGenerator : generatorAlgorithms) {
			particalConstraintGenerator.generateFirstRoundConstraints(bids, referencePoint, goodToCoalitionMap,
					corePaymentRule);
		}
