...
public class MarginalEconomyReferencePointFactory implements ReferencePointFactory {
	...
	public Payment computeReferencePoint(BundleValueBids<?> bids, Allocation allocation) {
		...
		for (Bidder bidder : allocation.getWinners()) {
			allocation.allocationOf(bidder).getBundle().getBundleEntries().stream().map(BundleEntry::getGood)
					.collect(Collectors.toCollection(LinkedHashSet::new)).forEach(g -> winnerMap.put(g, bidder));
		}
		...
		for (Bidder bidder : bids.getBidders()) {
			for (BundleExactValuePair bundleBid : bids.getBid(bidder).getBundleBids()) {
				BigDecimal bidPerGood = bundleBid.getAmount().divide(
						BigDecimal.valueOf(bundleBid.getBundle().getBundleEntries().size()), MathContext.DECIMAL64);
				// TODO: assumes availability of 1
				bundleBid.getBundle().getBundleEntries().stream().map(BundleEntry::getGood)
						.filter(good -> !Objects.equal(winnerMap.get(good), bidder))
						.forEach(good -> highestLosingBids.merge(good, bidPerGood, PrecisionUtils::max));
			}
		}
		...
		for (Bidder winner : allocation.getWinners()) {
			...
			BigDecimal referencePayment = bidderAllocation.getBundle().getBundleEntries().stream()
					.map(g -> highestLosingBids.getOrDefault(g.getGood(), BigDecimal.ZERO))
