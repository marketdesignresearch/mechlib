...
public abstract class BaseCorePaymentRule implements CorePaymentRule {
	...
	protected IMIP createProgram(Allocation allocation, Payment referencePoint) {
		...
		for (Bidder winner : allocation.getWinners()) {
			...
			Variable winnerPayment = new Variable(PaymentNorm.BIDDER + winner.getId(), VarType.DOUBLE,
	...
	protected MetaInfo addBlockingConstraint(IMIP program, BlockedBidders blockedBidders, Payment lastPayment) {
		Constraint blockingConstraint = new Constraint(CompareType.GEQ,
				blockedBidders.getBlockedAmount(lastPayment).doubleValue());
		...
		for (Bidder blockedBidder : blockedBidders.getNonTraitors()) {
			...
			if (blockedWinnerPayment == null) {
				throw new IllegalArgumentException(
						"Variable " + PaymentNorm.BIDDER + blockedBidder.getId() + " does not exist");
			}
		...
		if (blockingConstraint.size() > 0 && DoubleMath.fuzzyCompare(blockingConstraint.getConstant(), vcgPayments,
				PrecisionUtils.EPSILON.doubleValue()) > 0) {
