...
class IterativePartialSeparabilityConstraintsGenerator implements PartialConstraintGenerator {
	...
	public void generateConstraint(CorePaymentRule corePaymentRule, Graph<PotentialCoalition, DefaultEdge> graph,
		...
		for (Set<PotentialCoalition> subGraph : connectivityInspector.connectedSets()) {
			...
			Set<PotentialCoalition> subGraphWinners = ImmutableSet
					.copyOf(Sets.intersection(subGraph, winningAllocation.getPotentialCoalitions()));
			for (PotentialCoalition winner : subGraphWinners) {
				...
				while (blockedBidderSet.size() < subGraphWinners.size()
						&& !allreadyAddedCoalitions.contains(nStepBlockingBidders.getBlockingBidders())) {
	...
	private BlockedBidders findNStepBlockingBidders(Set<PotentialCoalition> startingPoints,
			NeighborCache<PotentialCoalition, DefaultEdge> index, Allocation blockingCoalition) {
