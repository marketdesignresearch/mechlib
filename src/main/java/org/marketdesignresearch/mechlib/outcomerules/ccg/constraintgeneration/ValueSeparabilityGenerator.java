...
public abstract class ValueSeparabilityGenerator implements PartialConstraintGenerator {
	...
	protected abstract void investigateSubCoalition(Set<PotentialCoalition> blockingBidders,
	...
	public void generateConstraint(CorePaymentRule corePaymentRule, Graph<PotentialCoalition, DefaultEdge> graph,
		...
		for (Set<PotentialCoalition> subgraph : connectivityInspector.connectedSets()) {
			// Compared to a previous implementation, the parallel aspect was removed here.
			// The problem was that
			// jgrapht's neighborsOf() method threw a ConcurrentModificationException in a
			// computeIfAbsent() call
			// when parallelized. This was possible due to a bug in JDK8 (which is why it
			// remained undetected), but not
			...
			AverageDistanceFromReference distancePerBidder = calcAdr(blockedBidders, priorResult.getPayment(),
					blockedBidders.getNonTraitors());
			addConstraints(blockedBidders, neighbors, priorResult.getPayment(), corePaymentRule, distancePerBidder);
		}
	}

	protected void addConstraints(BlockedBidders blockedBidders,
			NeighborCache<PotentialCoalition, DefaultEdge> neighbors, Payment priorPayment,
			CorePaymentRule corePaymentRule, AverageDistanceFromReference minadr) {
		if (!blockedBidders.getNonTraitors().isEmpty()) {
			...
			if (constraintCount.getAndIncrement() < MAX_CONSTRAINTS) {
				for (Bidder blockedBidder : blockedBidders.getBlockedBidders()) {
					investigateSubCoalition(blockedBidders.getBlockingBidders(), neighbors, blockedBidder, minadr,
							priorPayment, corePaymentRule);
				}
			}
		}
	...
	protected AverageDistanceFromReference calcAdr(BlockedBidders blockedBidders, Payment priorPayment,
			Set<? extends Bidder> superAllocationBlockedBidders) {
		BigDecimal constraintValue = blockedBidders.getBlockedAmount(priorPayment);
		BigDecimal totalReferencePayments = blockedBidders.getNonTraitors().stream().map(referencePayments::paymentOf)
				.map(BidderPayment::getAmount).reduce(BigDecimal.ZERO, BigDecimal::add);

		BigDecimal totalDistance = constraintValue.subtract(totalReferencePayments);

		return new AverageDistanceFromReference(totalDistance,
				ImmutableSet.copyOf(Sets.intersection(blockedBidders.getNonTraitors(), superAllocationBlockedBidders)));
	}
