...
public class CCGOutcomeRule implements OutcomeRule {
	...
	public CCGOutcomeRule(BundleValueBids<?> bids, Allocation allocation,
			BlockingAllocationFinder blockingCoalitionFactory, CorePaymentRule paymentRule,
	...
	public CCGOutcomeRule(BundleValueBids<?> bids, Allocation allocation, CorePaymentRule paymentRule,
			BlockingAllocationFinder blockingCoalitionFactory, Set<ConstraintGenerationAlgorithm> cgAlgorithms) {
		this.bids = bids;
	...
	private Outcome calculateCCGPrices(Allocation allocation) {
		...
		ConstraintGenerator constraintGenerator = ConstraintGenerationAlgorithm.getInstance(cgAlgorithms, bids,
				new Outcome(lastPayment, allocation), paymentRule);
		...
		do {
			...
			if (lastBlockingAllocationValue != null) {
				if (PrecisionUtils.fuzzyEquals(lastBlockingAllocationValue,
						blockingAllocation.getMostBlockingAllocation().getTotalAllocationValue(),
						PrecisionUtils.EPSILON.scaleByPowerOfTen(1))) {
					...
					if (comparisonPrecision.compareTo(BigDecimal.valueOf(0.1)) > 0)
						throw new IllegalStateException(
								"Precision of comparison of winner payments and block allocation decreases below 0.1 - This might be due to numerical issues. Please check your MIPs");
				} else {
			...
			log.debug("Blocking coalition found with value {}",
					blockingAllocation.getMostBlockingAllocation().getTotalAllocationValue());
		} while (PrecisionUtils.fuzzyCompare(blockingAllocation.getMostBlockingAllocation().getTotalAllocationValue(),
				totalWinnersPayments, comparisonPrecision) > 0
