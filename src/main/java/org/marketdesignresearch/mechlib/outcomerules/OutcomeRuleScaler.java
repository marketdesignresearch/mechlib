...
public class OutcomeRuleScaler implements OutcomeRule {
	...
	public Outcome getOutcome() {
		...
		for (Map.Entry<Bidder, BidderAllocation> e : originalOutcome.getAllocation().getTradesMap().entrySet()) {
			...
			tradesMap.put(e.getKey(),
					new BidderAllocation(value, e.getValue().getBundle(), e.getValue().getAcceptedBids()));
		}
		...
		Allocation newAllocation = new Allocation(tradesMap, originalBids, originalOutcome.getMetaInfo(),
				potentialCoalitions);
