...
public abstract class DefaultConvergenceStrategy implements ConvergenceStrategy {
	...
	public BundleBoundValueBid applyConvergenceStrategy(ConvergenceInteraction query, Auction<?> auction) {
		...
		for (Bundle bundle : query.getBundles()) {
			...
			BigDecimal upperUncertainty = epsilon
					.multiply(BigDecimal.valueOf(this.getNextGuassianLikeDouble(BidderRandom.INSTANCE.getRandom())));
			upperUncertainty = upperUncertainty.min(epsilon);

			BigDecimal upperBound = getValueFunction().getValue(bundle).divide(
					BigDecimal.ONE.subtract(upperUncertainty), Math.max(10, upperUncertainty.scale()),
					RoundingMode.DOWN);
			upperBound = upperBound.max(currentPair.getLowerBound().divide(BigDecimal.ONE.subtract(epsilon),
					Math.max(10, upperUncertainty.scale()), RoundingMode.DOWN));
