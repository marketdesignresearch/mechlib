...
public class UnitDemandBidder implements Bidder, Serializable {
	...
	public LinkedHashSet<Bundle> getBestBundles(Prices prices, int maxNumberOfBundles, boolean allowNegative) {
		return Sets.powerSet(new LinkedHashSet<>(goodsOfInterest)).stream().map(Bundle::of)
				.sorted((a, b) -> getValue(b).subtract(prices.getPrice(b).getAmount())
						.compareTo(getValue(a).subtract(prices.getPrice(a).getAmount())))
				.filter(bundle -> allowNegative || value.subtract(prices.getPrice(bundle).getAmount()).signum() > -1)
				.limit(maxNumberOfBundles).collect(Collectors.toCollection(LinkedHashSet::new));
	}
