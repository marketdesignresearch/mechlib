...
public class DIARVariant2Refiner extends AutomatedRefiner<DIARVariant2Refinement>{

	public static BigDecimal roundingDelta = BigDecimal.valueOf(1E-8d);
	
	@Override
	public BundleBoundValueBid refineBids(DIARVariant2Refinement type, ValueFunction b, BundleBoundValueBid activeBids,
		...
		List<ImmutablePair<Bundle, BigDecimal>> errors = this.getDIARErrorOrder(activeBids, prices, provisionalAllocation);
		...
		for(ImmutablePair<Bundle, BigDecimal> error : errors) {
			...
			if(refinedBid.getLowerBound().subtract(activeBid.getLowerBound()).compareTo(type.getEpsilon().add(roundingDelta))>=0) {
				// done with refinement
				break;
			}
			// check if the lower bond can be increased by epsilon
			if(activeBid.getLowerBound().add(type.getEpsilon()).add(roundingDelta).compareTo(b.getValue(error.left)) <= 0) {
				...
				returnBid.addBundleBid(new BundleBoundValuePair(newLowerBound, refinedBid.getUpperBound(), refinedBid.getBundle(), refinedBid.getId()));
				// done with refinement
				break;
			}
			else {
