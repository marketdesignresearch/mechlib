 ...
 */
public class AdditiveOverbiddingTransformation<S extends ComparableTransformation<S>>
		implements MultiDimensionalComparableTransformation<AdditiveOverbiddingTransformation<S>> {
	public static final AdditiveOverbiddingTransformation<ShaveTransformation> TRUTHFUL = new AdditiveOverbiddingTransformation<>(
			ShaveTransformation.TRUTHFUL, BigDecimal.ZERO, ImmutableSet.of());
	...
	public BundleExactValueBid apply(BidTransformableValueFunction combinatorialValueFunction) {
		...
		if (overBid.signum() != 0) {
			BigDecimal bidSoFar = bid.getBundleBids().stream().map(BundleExactValuePair::getAmount)
