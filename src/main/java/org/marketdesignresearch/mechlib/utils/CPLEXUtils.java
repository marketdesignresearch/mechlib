...
/**
 * A wrapper around the solving logic. By default, all solves in MechLib are
 * solved through this singleton, which allows setting some parameters that are
 * applied consistently among all MIPs that are solved inside the MechLib. E.g.,
 * you can specify a thread count here, and this will be applied to all MIPs
 * that are to be solved. Parameters that are set on {@link IMIP}-level are NOT
 * overwritten.
 *
 * This wrapper also exposes some methods to set default parameters in a way
 * they have been set in experiments before.
 */
@Slf4j
public enum CPLEXUtils {
	...
	/**
	 * Solves the program. It applies the solve parameters that are specified in
	 * {@link #solveParamMap} only if they are not already specified in the program
	 * itself. Careful: JOpt has a default parameter logic as well, so often the
	 * program already includes JOpt-specific default parameters, which will not be
	 * overwritten here. To be sure, you can call {@link IMIP#clearSolveParams()}
	 * before setting your own parameters
	 *
	...
	/**
	 * You can add additional (or override previously added) default parameters that
	 * are then applied to any future program that does not define these solve
	 * parameters by itself already.
	 * 
	...
	/**
	 * A helper function to initialize the solver with some reasonable default
	 * parameters. Has to be actively called by the user. This sets the following:
	 * <ul>
	 * <li>DISPLAY_OUTPUT: according to the log level - only if the debug log level
	 * is enabled, it's set to true</li>
	 * <li>THREADS: 1</li>
	...
	/**
	 * A helper function to initialize the solver with default parameters that were
	 * previously used in experiments of very specific CCG-price calculation MIPs
	 */
