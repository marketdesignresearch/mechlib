...
public abstract class WinnerDetermination implements AllocationRule {
	...
	/**
	 * Defines the time limit for the solver. What happens after the time limit is
	 * defined via {@link #setAcceptSuboptimal(boolean)}.
	 *
	 ...
	 *
	 * @param acceptSuboptimal true: accept a suboptimal solution at timeout; false:
	 *                         throw an exception at timeout
	 */
	@Setter
	...
	private List<Allocation> solveIntermediateSolutions(IMIPResult result) {
		if (result.getPoolSolutions() != null && !result.getPoolSolutions().isEmpty()) {
			...
			return result.getPoolSolutions().stream().filter(sol -> sol.getObjectiveValue() > lowerBound)
					.filter(Predicate.isEqual(result).negate()).distinct()
					.sorted(Comparator.comparingDouble(PoolSolution::getObjectiveValue).reversed())
					.map(this::adaptMIPResult).collect(Collectors.toList());
		} else {
	...
	public List<Allocation> getBestAllocations(int k, boolean allowNegative) {
		if (k == 1)
			return Lists.newArrayList(getAllocation());
		// This invalidates the current result, because otherwise previously collected
