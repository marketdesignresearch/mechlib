...
public class SimultaneousAuction extends Auction<BundleExactValuePair> {
	 ...
	 * return new HashMap<>(); Map<Bidder, List<Bundle>> restrictedBids = new
	 * HashMap<>(); List<Bundle> bundleList =
	 * getDomain().getGoods().stream().map(Bundle::of).collect(Collectors.toList());
	 * getDomain().getBidders().forEach(bidder -> restrictedBids.put(bidder,
	 * bundleList)); return restrictedBids; }
	 */
}
