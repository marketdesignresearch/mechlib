...
public class DIARValidator extends ActivityRuleValidator<DIARRefinement> {
	...
	@Override
	public void validateRefinement(DIARRefinement type, BundleBoundValueBid activeBids, BundleBoundValueBid refinedBids,
	...
	private BigDecimal highestErrorReductionPossible(BundleValueBid<BundleBoundValuePair> activeBids,
		...
		if (provisionalAllocation != null && !(provisionalAllocation.getTotalAmount() == 0))
			provisionalError = activeBids.getBidForBundle(provisionalAllocation).getLowerBound()
					.subtract(bidderPrices.getPrice(provisionalAllocation).getAmount());
		...
		for (BundleBoundValuePair activeBid : activeBids.getBundleBids()) {
			BigDecimal error = this.perturbedValuation(activeBid, provisionalAllocation)
					.subtract(bidderPrices.getPrice(activeBid.getBundle()).getAmount()).subtract(provisionalError);
			...
			if (error.compareTo(highestErrorImproved) > 0) {
				if (!activeBid.getBundle().equals(provisionalAllocation)) {
					...
					if (provisionalAllocation != null && !(provisionalAllocation.getTotalAmount() == 0)) {
						range = range.add(refinedBids.getBidForBundle(provisionalAllocation).getUpperBound()
								.subtract(refinedBids.getBidForBundle(provisionalAllocation).getLowerBound()));
					}
		...
		// empty bundle
		if (provisionalAllocation != null && !(provisionalAllocation.getTotalAmount() == 0)) {
			BigDecimal error = BigDecimal.ZERO.subtract((activeBids.getBidForBundle(provisionalAllocation)
					.getLowerBound().subtract(bidderPrices.getPrice(provisionalAllocation).getAmount())));
			if (error.compareTo(highestErrorImproved) > 0) {
				BigDecimal range = refinedBids.getBidForBundle(provisionalAllocation).getUpperBound()
						.subtract(refinedBids.getBidForBundle(provisionalAllocation).getLowerBound());
				if (range.compareTo(epsilon) >= 0) {
					highestErrorImproved = error;
				}
	...
	private BigDecimal highestErrorReduced(BundleValueBid<BundleBoundValuePair> activeBids,
		...
		for (BundleBoundValuePair activeBid : activeBids.getBundleBids()) {
			...
			if (error.compareTo(highestErrorImproved) > 0) {
				BigDecimal errorReduction = this.perturbedValuation(activeBid, provisionalAllocation).subtract(this
						.perturbedValuation(refinedBids.getBidForBundle(activeBid.getBundle()), provisionalAllocation));
				if (provisionalAllocation != null && !(provisionalAllocation.getTotalAmount() == 0))
		...
		if (provisionalAllocation != null && !(provisionalAllocation.getTotalAmount() == 0)) {
			BigDecimal error = BigDecimal.ZERO.subtract(activeBids.getBidForBundle(provisionalAllocation)
					.getLowerBound().subtract(bidderPrices.getPrice(provisionalAllocation).getAmount()));
			if (error.compareTo(highestErrorImproved) > 0) {
				BigDecimal errorReduction = BigDecimal.ZERO
						.subtract(activeBids.getBidForBundle(provisionalAllocation).getLowerBound())
						.add(refinedBids.getBidForBundle(provisionalAllocation).getLowerBound());
				if (errorReduction.compareTo(epsilon) >= 0) {
					highestErrorImproved = error;
