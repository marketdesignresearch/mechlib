...
public class BoundSupportVectorMIP extends SupportVectorMIP<BundleBoundValueBid>{
	...
	protected IMIP createMip() {
		...
		for(BundleBoundValuePair bid : this.getBid().getBundleBids()){
			...
			double spread = bid.getUpperBound().subtract(bid.getLowerBound()).divide(BigDecimal.valueOf(2),RoundingMode.HALF_UP).doubleValue();
			double mean = bid.getUpperBound().add(bid.getLowerBound()).divide(BigDecimal.valueOf(2), RoundingMode.HALF_UP).doubleValue();
			mipWrapper.addObjectiveTerm(-spread-this.getInsensitivityThreshold(), labeledDataVariables.get(indexQuery));
			mipWrapper.addObjectiveTerm(-spread-this.getInsensitivityThreshold(), labeledDataVariables.get(-indexQuery));
		...
		for (BundleBoundValuePair bv1 : this.getBid().getBundleBids()){
			...
			for (BundleBoundValuePair bv2 : this.getBid().getBundleBids()){
				...
				mipWrapper.addObjectiveTerm(-0.5*kernelVal,labeledDataVariables.get(indexQuery),labeledDataVariables.get(indexQuery2));
				mipWrapper.addObjectiveTerm(-0.5*kernelVal,labeledDataVariables.get(-indexQuery),labeledDataVariables.get(-indexQuery2));
				mipWrapper.addObjectiveTerm(+0.5*kernelVal,labeledDataVariables.get(indexQuery),labeledDataVariables.get(-indexQuery2));
				mipWrapper.addObjectiveTerm(+0.5*kernelVal,labeledDataVariables.get(-indexQuery),labeledDataVariables.get(indexQuery2));
			}
