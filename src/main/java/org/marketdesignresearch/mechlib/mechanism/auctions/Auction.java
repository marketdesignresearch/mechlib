...
/**
 * An (iterative) bundle value bid Auction. An auction is build with different
 * consecutive AuctionPhases. An AuctionPhase can consist of different rounds.
 * Each AuctionPhase is responsible to drive the auction. Auction Rounds are
 * following this sequence:
 * 
 * <ol>
 * <li>The AuctionPhase is called to create an AuctionRoundBuilder that is
 * responsible for this round. The AuctionRoundBuilder holds an interaction for
 * each bidder that is passed through by the auction
 * <li>Each bidder can access the interaction and submit bids
 * <li>(optional) by {@link #advanceRound()} the proposed bids will be submitted
 * if no bids were submitted before for a bidder (which were generated by a
 * bidders strategy {@link InteractionStrategy} and close the round.
 * <li>The AuctionRound can be also just closed by {@link #closeRound()} without
 * submitting the proposed bids
 * <li>The AuctionRoundBuilder is called to close this round and create an
 * AuctionRound which is stored and accessible directly from the Auction.
 * <li>If the AuctionPhase indicates that no more rounds follow the next
 * AuctionPhase is activated. If no AuctionPhase is left the Auction closes.
 * </ol>
 * 
 * All rounds are stored in the auction round list {@linkplain #getRound(int)}
 * with the number of rounds {@linkplain #getNumberOfRounds()}. An AuctionRound
 * might hold all relevant details including submitted bids.
 * 
 * Each auction can be reset to any previous round and continued form there
 * {@link #resetToRound(int)}. Note that therefore all state should be captured
 * with an AuctionRound. An AuctionPhase should never contain any state
 * information because an AuctionPhase is not reset!
 * 
 * Note that an Auction holds a round dependant Random Object that might be used
 * to in an AuctionPhase, AuctionRoundBuilder or in an InteractionStrategy. When
 * no manual bids are submitted (i.e. all bidders use solely an
 * InteractionStrategy and the round advances with {@link #advanceRound()} the
 * outcome of this auction should be completely reproducible. (Also if the
 * auction is {@link #resetToRound(int)} and altered (i.e. changed an upcoming
 * AuctionPhase) the result will be the same, as if the auction was run from the
 * beginning directly.
 * 
 * If you are interested in creating an auction with exact value bids you may
 * consider extending {@link ExactValueAuction}.
 * 
 * FIXME: MipInstrumentation should be handled properly.
 * 
...
public abstract class Auction<BB extends BundleValueBids<?>> extends Mechanism implements AuctionInstrumentationable {
	...
	/**
	 * The current round of the active AuctionPhase (starting from 0). For each new
	 * AuctionPhase this counter will be reset.
	 */
	@Getter
	...
	/**
	 * The AuctionRoundBuilder of the current running round. This attribute will be
	 * set to null if the auction has finished
	 */
	protected AuctionRoundBuilder<BB> current;

	/**
	 * Creates a new Auction and starts the first round. More AuctionPhases can be
	 * added later.
	 * 
	 * This Constructor sets a random seed for the round random generator. Results
	 * of Auctions are therefore not reproducible if the round random object is used
	 * any time.
	 * 
	 ...
	 * @param outcomeRuleGenerator the outcome rule generator
	 ...
	 * @throws NoSuchElementException if there is no bidder in the domain of this
	 *                                auction with the given UUID
	 */
	 ...
	 * @throws NoSuchElementException if there is no good in the domain of this
	 *                                auction with the given name
	 */
	...
	/**
	 * Checks whether this auction has finished
	 * 
	 ...
	 * AuctionPhase has finished it moves to the next AuctionPhase or it closes the
	 * Auction if there is no next AuctionPhase.
	 */
	protected void prepareNextAuctionRoundBuilder() {
		...
		if ((this.phases.size() == this.currentPhaseNumber) || (maxRounds > 0 && getNumberOfRounds() >= maxRounds)) {
			current = null;
			// Start next round
		} else {
	...
	/**
	 * Returns all submitted bids so far. At the end of the auction this will return
	 * all bids submitted during the auction.
	 * 
	...
	/**
	 * Returns the bids of the last completed round. To access the bids already made
	 * in the current active round you need to access them through the interactions
	 * 
	 ...
	 * By default, the bids for the auction results are aggregated in each round
	 * 
	 * @param generator the OutcomeRuleGenerator that should be used to generate the
	 *                  outcome
	...
	/**
	 * Computes the temporary result including bids submitted in the current round
	 * using the given OutcomeRuleGenerator.
	 * 
	 * @param generator The OutcomeRuleGenerator that is used to compute the outcome
	 * @return the outcome
	 */
	...
	/**
	 * Get the type name of the current AuctionPhase
	 * 
