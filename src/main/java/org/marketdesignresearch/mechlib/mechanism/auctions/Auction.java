 ...
 * 
 * 1. The AuctionPhase is called to create an AuctionRoundBuilder that is
 * responsible for this round The AuctionRoundBuilder holds an interaction for
 * each bidder that is passed through by the auction 2. Each bidder can access
 * the interaction and submit bids 3. (optional) by {@link #advanceRound()} the
 * proposed bids will be submitted if no bids were submited before for a bidder
 * (which were generated by a bidders strategy {@link InteractionStrategy} and
 * close the round. 4. The AuctionRound can be also just closed by
 * {@link #closeRound()} without submitting the proposed bids 5. The
 * AuctionRoundBuilder is called to close this round and create an AuctionRound
 * which is stored and accessible directly from the Auction. 6. If the
 * AuctionPhase indicates that no more rounds follow the next AuctionPhase is
 * activiated If no AuctionPhase is left the Auction closes.
 * 
...
public abstract class Auction<BB extends BundleValueBids<?>> extends Mechanism implements AuctionInstrumentationable {
	 ...
	 * of Auctions are threfore not reproducible if the round random object is used
	 * any time.
	 * 
