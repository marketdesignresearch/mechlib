 ...
 * 
 * 1. the efficiency of the allocation (with respect to given bids) should be
 * guaranteed to be higher than the specified efficiency tolerance. 2. the
 * relativ difference between lower and upper bound social welfare of the
 * allocation must be within lower than the specific maximal gap.
 * 
...
public class IntervalSizeEfficiencyInfoCreator extends EfficiencyGuaranteeEfficiencyInfoCreator {
	...
	/**
	 * @param efficiencyTolerance the minimum efficiency guarantee to consider an
	 *                            allocation as efficient.
	 * @param allowedUncertainty  the maximal relativ gap between lower and upper
	...
	public boolean hasConverged(LinkedHashMap<ElicitationEconomy, ElicitationEconomyEfficiency> info,
		...
		BigDecimal upperBoundValue = allocation.getTradesMap().entrySet().stream()
				.map(e -> e.getValue().getBundle().equals(Bundle.EMPTY) ? BigDecimal.ZERO
						: bids.getBid(e.getKey()).getBidForBundle(e.getValue().getBundle()).getUpperBound())
				.reduce(BigDecimal.ZERO, BigDecimal::add);

		return upperBoundValue.subtract(lowerBoundValue)
				.divide(upperBoundValue, allocationValueGap.scale() + 1, RoundingMode.HALF_UP)
				.compareTo(allocationValueGap) <= 0 && super.hasConverged(info, bids);
	}
