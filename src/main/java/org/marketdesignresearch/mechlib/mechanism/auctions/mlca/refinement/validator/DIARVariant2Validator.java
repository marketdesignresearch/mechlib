...
@Slf4j
public class DIARVariant2Validator extends ActivityRuleValidator<DIARVariant2Refinement>{

	@Override
	public void validateRefinement(DIARVariant2Refinement type, BundleBoundValueBid activeBids, BundleBoundValueBid refinedBids,
	...
	private BigDecimal highestErrorReduced(BundleValueBid<BundleBoundValuePair> activeBids,
		...
		for (BundleBoundValuePair activeBid : activeBids.getBundleBids()) {
			...
			if (error.compareTo(highestErrorImproved) > 0) {
				BigDecimal errorReduction = refinedBids.getBidForBundle(activeBid.getBundle()).getLowerBound().subtract(activeBid.getLowerBound()); 
